...
<?php
  $query_DatosPub1 = sprintf("SELECT * FROM publications ORDER BY site ASC");
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "formrequest")) {
  $year = date("Y");
	$month = date("m");
	$day = date("d");
  $insertSQL = sprintf("INSERT INTO publications(date, year, month, day, time, foto, title, content, site, status) 
